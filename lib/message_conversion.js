...
class MessageConversion {
    ...
    convertFromPiotMessage(topic, data) {
        ...
        console.log("data", this.piotMessage.data)
    ...
    convertFromMessage(path, value) {
        ...
        this.messages[0].path = pathLevels.join('/');
    ...
    convertToPiotMessage(offset) {
        let buffer = new Uint8Array(1 + offset + this.piotMessage.topic.length + this.piotMessage.length);
        // topic
        ...
        Bytes.bufferCopy(buffer, offset + 9, this.piotMessage.data, 0, this.piotMessage.length);
        return buffer;
    }
    ...
    _convertToPiot () {
        ...
        if (MessDefs.hasOwnProperty(this.device.type)) {
            ...
            for (this.piotMessage.number of Object.keys(deviceDefinition.topicNumbers)) {
                this.messages[0].index = -1;

                // do we have an array of topics
                if (deviceDefinition.topicNumbers[this.piotMessage.number].hasOwnProperty('topics')) {
                    this.messages[0].index = 0;
                    for (const testMessageDefinition of deviceDefinition.topicNumbers[this.piotMessage.number].topics) {
                        if (testMessageDefinition.name === this.messages[0].name) {
                            messageDefinition = testMessageDefinition;
                            break;
                        }
                        this.messages[0].index++;
                    }
                }
                else if (deviceDefinition.topicNumbers[this.piotMessage.number].name === this.messages[0].name) {
                    messageDefinition = deviceDefinition.topicNumbers[this.piotMessage.number];
                }
        ...
        if (messageDefinition === undefined) {
            ...
            if (Bytes.isIterable(this.messages[0].value)) {
                this.piotMessage.length = this.messages[0].value.length;
                this.piotMessage.message[0].length = this.messages[0].value.length;
        ...
        if (this.messages[0].index > -1) {
            // encode index as first byte of message payload
            this.messages[0].length = this._getMessageSize(messageDefinition) + 1;
            this.piotMessage.data = new Uint8Array(this.messages[0].length);
            this.piotMessage.data[0] = this.messages[0].index;
    ...
    _convertDataFromPiot () {
        ...
        for(const messageDefinition of messageDefinitions) {
            ...
            if (this.piotMessage.data instanceof Uint8Array && (this.piotMessage.data.length - subOffset) >= length) {
                try {
                    value = this._convertBytes(messageDefinition, this.piotMessage.data, subOffset);
                } catch (err) {
                    value = this.piotMessage.data.slice(subOffset, length);
                }
            }
            ...
            this.messages.push({
                ...
                length: length,
                value: value,
                index: commandMapped ? this.messages.length : -1,
            });
            subOffset += length;
        };
    }
